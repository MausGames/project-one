...
void cConfigMenu::CheckValues()
{
    ...
    #define __CHECK_INPUT(n,a,c) {coreBool A = false; for(coreUintW i = 0u; i < (c); ++i) A = A || (a); bSave = bSave || A; m_aLabel[ENTRY_ ## n].SetColor3(A ? g_pMenu->GetHighlightColor() : COLOR_MENU_WHITE);}
    {
        ...
        __CHECK_VALUE(GAME_BACKROTATION,     m_BackRotation          .GetCurValue()   !=  g_OldConfig.Game.iBackRotation)
        __CHECK_VALUE(GAME_BACKSPEED,        m_BackSpeed             .GetCurValue()   !=  g_OldConfig.Game.iBackSpeed)
        __CHECK_VALUE(GAME_UPDATEFREQ,       m_UpdateFreq            .GetCurValue()   !=  g_OldConfig.Game.iUpdateFreq)
