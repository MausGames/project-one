...
// constant values
const vec3 c_v3Blue = vec3(0.0, 0.56, 0.56);   // default surface color
...
void FragmentMain()
{
    ...
    vec3  v3BumpNormal   = v3BumpNormal1 + v3BumpNormal2;
          v3BumpNormal   = normalize(vec3(v3BumpNormal.xy, v3BumpNormal.z / 2.0));
          v3BumpNormal   = normalize(v3BumpNormal + vec3(v3BumpNormalDrop.x, -v3BumpNormalDrop.y, 0.0));
    ...
    float v1ReflFactor  = max(0.0, dot(v3ReflNormal, v3BumpNormal));
          v1ReflFactor  = 0.4 * min(coreGGX(v1ReflFactor, 0.008), 1.0);
