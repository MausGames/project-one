...
void cMossBackground::__MoveOwn()
{
    ...
    if((fPrevDelay < 0.0f) && (m_fThunderDelay >= 0.0f))
    {
        ...
        m_apThunder[m_iThunderIndex]->PlayRelative(this, 0.0f, 1.0f, false);
    }
    ...
    for(coreUintW i = 0u; i < ARRAY_SIZE(m_apThunder); ++i)
    {
        if(m_apThunder[i]->EnableRef(this))
            m_apThunder[i]->SetVolume(g_pEnvironment->RetrieveTransitionBlend(this) * g_CurConfig.Audio.fAmbientVolume);
    }

    // 
    if(m_pRainSound->EnableRef(this))
        m_pRainSound->SetVolume(g_pEnvironment->RetrieveTransitionBlend(this) * g_CurConfig.Audio.fAmbientVolume);
}