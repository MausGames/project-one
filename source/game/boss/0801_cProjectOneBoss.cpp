...
// - purple:  transition, phase, phase, phase, phase
...
void cProjectOneBoss::__KillOwn(const coreBool bAnimated)
{
    ...
    // 
    for(coreUintW i = 0u; i < PROJECTONE_FRAGMENTS; ++i)
        this->__DisableFragment(i, bAnimated);
...
void cProjectOneBoss::__MoveOwn()
{
    ...
    else if(m_iPhase == 2u)
    {
        if(PHASE_BEGINNING2)
        {
            ...
            g_pSpecialEffects->PlaySound(coreVector3(0.0f,1.3f,0.0f) * FOREGROUND_AREA3, 1.0f, 1.0f, SOUND_SHIP_FLY);
        }

        PHASE_CONTROL_TIMER(0u, 0.4f, LERP_BREAK)
        {
            this->DefaultMoveLerp(coreVector2(0.0f,1.3f), coreVector2(0.0f,0.0f), fTime);
    ...
    else if(m_iPhase == 11u)
    {
        ...
        if(PHASE_MAINTIME_POINT(6.0f))
        {
            ...
            if(std::all_of(piFragment, piFragment + 8u, [](const coreUint8 A) {return A;}))
            {
                m_aiCounter[FRAGMENT_PHASE] = 1;
                ...
                if(g_pGame->IsMulti())
                {
                    ...
                    g_pSpecialEffects->CreateBlowColor(pOther->GetPosition(), coreVector3(m_avVector[FINAL_PUSH].xy(), 0.0f), SPECIAL_BLOW_SMALL, COLOR_ENERGY_WHITE);
                }
            }
        }
    ...
    else if(m_iPhase == 12u)
    {
        ...
        for(coreUintW i = 0u; i < PROJECTONE_FRAGMENTS; ++i)
        {
            ...
            const coreFloat fTime = CLAMP01((m_fPhaseTime - 8.0f - I_TO_F(i) * 0.3f) * 0.7f);
    ...
    else if(m_iPhase == 13u)
    {
        ...
        PHASE_CONTROL_TIMER(0u, 0.2f, LERP_SMOOTH)
        {
            ...
            this->DefaultMoveLerp(m_vLastPosition, coreVector2(-fSide / FOREGROUND_AREA.x, 0.0f), fTime);

            if(g_pGame->IsMulti())
            {
                pOther->SetPosition   (coreVector3(LERP(coreVector2(60.0f,-20.0f), coreVector2(fSide, 0.0f), fTime), 0.0f));
        ...
        const coreFloat fSpawn = LERPB(FOREGROUND_AREA.x * 1.9f * SQRT2, 15.0f, MIN1(fTime)) - LERPS(0.0f, 15.0f, MIN1(fTime - 1.1f));
        ...
        if(g_pGame->IsMulti())
        {
            const coreVector2 vPos = pOther->GetPosition().xy() + coreVector2(0.0f,1.0f) * fSpawn;
        ...
        if(fSpawn <= 0.1f)
        {
            ...
            g_pSpecialEffects->PlaySound(this->GetPosition(), 1.0f, 1.0f, SOUND_ENEMY_EXPLOSION_07);

            if(g_pGame->IsMulti())
            {
                g_pSpecialEffects->MacroExplosionColorBig(pOther->GetPosition(), COLOR_ENERGY_WHITE);
                g_pSpecialEffects->PlaySound(pOther->GetPosition(), 1.0f, 1.0f, SOUND_ENEMY_EXPLOSION_07);
            }
    ...
    {
        PHASE_CONTROL_TIMER(0u, 0.9f, LERP_BREAK)
        {
            cPlayer* pPlayer = g_pGame->GetPlayer(0u);

            const coreVector2 vDir = coreVector2::Direction(LERP(m_vLastDirection.xy().Angle(), 0.0f*PI, fTime));

            pPlayer->SetDirection(coreVector3(vDir,             0.0f));
            pOther ->SetDirection(coreVector3(vDir.InvertedX(), 0.0f));
    ...
    else if(m_iPhase == 33u)
    {
        ...
        if(PHASE_BEGINNING2)
        {
            ...
            g_pSpecialEffects->PlaySound(m_aShield[iIndex].GetPosition(), 1.0f, 1.0f, SOUND_PLACEHOLDER);   // shield burst      
        }
    ...
    if(!m_bDead)
    {
        ...
        // 
        const coreFloat fFullThrust = MAX0(m_fSmoothThrust) * 0.055f;
...
void cProjectOneBoss::__MoveYellow()
{
    ...
    else if(m_iPhase == 52u)
    {
        ...
        PHASE_CONTROL_TIMER(0u, 0.6f, LERP_LINEAR)
        {
            ...
            {
                g_pSpecialEffects->CreateSplashColor(this->GetPosition(), 0.0f, 1u, COLOR_ENERGY_WHITE, false, true);
                g_pSpecialEffects->CreateSplashColor(vOther1,             0.0f, 1u, COLOR_ENERGY_WHITE, false, true);
                g_pSpecialEffects->CreateSplashColor(vOther2,             0.0f, 1u, COLOR_ENERGY_WHITE, false, true);
            });
                ...
                for(coreUintW i = 0u; i < PROJECTONE_CLONES; ++i)
                {
                    ...
                    if(PHASE_MAINTIME_AFTER(1.2f))
                    {
                        Core::Manager::Object->TestCollision(TYPE_BULLET_PLAYER, &m_aClone[i], [&](const cBullet* pBullet, const coreObject3D* pClone, const coreVector3 vIntersection, const coreBool bFirstHit)
                        {
                            ...
                            g_pSpecialEffects->CreateSplashSmoke(m_aClone[i].GetPosition(), 25.0f, 40u, coreVector3(1.0f,1.0f,1.0f));
                            g_pSpecialEffects->CreateSplashColor(m_aClone[i].GetPosition(), SPECIAL_SPLASH_SMALL, COLOR_ENERGY_WHITE);
...
void cProjectOneBoss::__MoveMagenta()
{
    ...
    else if(m_iPhase == 81u)
    {
        ...
        PHASE_CONTROL_TICKER(0u, 0u, 6.0f, LERP_LINEAR)
        {
            ...
            for(coreUintW j = 0u; j < 5u; ++j)
            {
                ...
                const coreVector2 vDir = coreVector2::Direction(DEG_TO_RAD((I_TO_F((j * 2u + 1u) % 5u) - 2.0f) * 3.0f) + fBase);
    ...
    else if(m_iPhase == 84u)
    {
        ...
        const coreVector2 vNewAxis = coreVector2::Direction(AngleLerp(m_fGameAngle, 0.0f, BLENDB(MIN1(m_fPhaseTime * 0.5f))));
    ...
    else if(m_iPhase == 108u)
    {
        PHASE_CONTROL_TIMER(0u, 1.0f, LERP_BREAK_REV)
        {
            ...
            if(PHASE_FINISHED)
            {
                ...
                g_pSpecialEffects->CreateSplashColor(coreVector3(-1.1f,0.0f,0.0f) * FOREGROUND_AREA3, SPECIAL_SPLASH_SMALL, COLOR_ENERGY_WHITE);
                g_pSpecialEffects->ShakeScreen(SPECIAL_SHAKE_SMALL);
                g_pSpecialEffects->PlaySound(coreVector3(-1.1f,0.0f,0.0f) * FOREGROUND_AREA3, 0.5f, 1.5f, SOUND_EFFECT_SHAKE);
            }
        });
    }
    ...
    else if(m_iPhase == 122u)
    {
        ...
        if(PHASE_MAINTIME_AFTER(4.0f))
        {
            PHASE_CONTROL_TIMER(4u, 0.25f, LERP_LINEAR)
            {
                ...
                if(PHASE_BEGINNING)
                {
                    g_pSpecialEffects->PlaySound(this->GetPosition(), 1.0f, 1.0f, SOUND_SHIP_FLY);
                }
    ...
    else if(m_iPhase == 131u)
    {
        ...
        PHASE_CONTROL_TICKER(0u, 0u, 0.7f * fSpeed, LERP_LINEAR)
        {
            ...
            for(coreUintW j = 8u; j--; )
            {
                ...
                g_pGame->GetBulletManagerEnemy()->AddBullet<cSpearBullet>(5, 0.4f, this, vPos, vDir)->ChangeSize(1.8f);
            }

            g_pSpecialEffects->CreateSplashColor(this->GetPosition(), SPECIAL_SPLASH_TINY, COLOR_ENERGY_YELLOW);
            g_pSpecialEffects->PlaySound(this->GetPosition(), 1.0f, 1.0f, SOUND_WEAPON_ENEMY);
        });
    ...
    else if(m_iPhase == 137u)
    {
        PHASE_CONTROL_TIMER(0u, 0.5f, LERP_LINEAR)
        {
            if(m_aiCounter[JUMP_COUNT] % 2 == 1u)
            {
                ...
                PHASE_CONTROL_TICKER(1u, 0u, 4.0f, LERP_LINEAR)
                {
                    const coreVector2 vBase = this->GetPosition().xy();
                    const coreVector2 vDir  = AlongCrossNormal(this->GetDirection().xy());
                    const coreVector2 vTan  = vDir.Rotated90();

                    const coreUintW iCheck = ((iTick / (g_pGame->IsEasy() ? 4u : 2u) + m_aiCounter[JUMP_COUNT] / 2u) % 2u);

                    for(coreUintW j = 9u; j--; )
                    {
                        if(iCheck == ((j / 3u) % 2u)) continue;

                        const coreVector2 vPos = vBase + vTan * ((I_TO_F(j) - 4.0f) * 3.0f);

                                               g_pGame->GetBulletManagerEnemy()->AddBullet<cViewBullet>(5, 0.9f, this, vPos,  vDir)->ChangeSize(1.6f);
                        if(!g_pGame->IsEasy()) g_pGame->GetBulletManagerEnemy()->AddBullet<cViewBullet>(5, 0.9f, this, vPos, -vDir)->ChangeSize(1.6f);
                    }

                    g_pSpecialEffects->CreateSplashColor(coreVector3(vBase, 0.0f), 25.0f, 5u, COLOR_ENERGY_MAGENTA);
            ...
            else
            {
                ...
                PHASE_CONTROL_TICKER(1u, 0u, 24.0f, LERP_LINEAR)
                {
                    ...
                    g_pGame->GetBulletManagerEnemy()->AddBullet<cConeBullet>(5, 0.5f, this, vPos,  vDir)->ChangeSize(1.7f);
                    g_pGame->GetBulletManagerEnemy()->AddBullet<cConeBullet>(5, 0.5f, this, vPos, -vDir)->ChangeSize(1.7f);
...
void cProjectOneBoss::__MoveWhite()
{
    ...
    else if(m_iPhase == 142u)
    {
        ...
        else if(fValue < 0.6f)
        {
            PHASE_CONTROL_TICKER(1u, 0u, 15.0f, LERP_LINEAR)
            {
                ...
                for(coreUintW j = 12u; j--; )
                {
                    ...
                    this->__AddRainbowBullet((j + iTick) % 8u, (iLine % 2u) ? 4 : 3, 1.5f, vPos, vDir)->AddStatus(BULLET_STATUS_IMMORTAL);
                }
        ...
        const coreMatrix2 mRota2 = coreMatrix3::Rotation(TIME * -STEP(0.4f, 0.6f, fValue)).m12();
        ...
        if(fValue >= 1.0f)
        {
            ...
            else
            {
                ...
                this->AddStatus(ENEMY_STATUS_GHOST_PLAYER | ENEMY_STATUS_WORTHLESS);
...
void cProjectOneBoss::__EndMission(const coreBool bAnimated, const coreBool bReturn)
{
    ...
    if(iID == cViridoMission::ID)
    {
        ...
        // 
        for(coreUintW i = 0u; i < VIRIDO_HINTS; ++i)
            pVirido->DisableHint(i, bAnimated);
    }
