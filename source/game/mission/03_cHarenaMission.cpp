...
, m_iInsanity    (0u)
, m_fAnimation   (0.0f)
, m_bStory       (!HAS_BIT_EX(REPLAY_WRAP_PROGRESS_STATE, STATE_STORY_HARENA) && (g_pReplay->GetMode() != REPLAY_MODE_PLAYBACK))
{
    // 
    m_apBoss[0] = &m_Tiger;
...
void cHarenaMission::__MoveOwnAfter()
{
    ...
    for(coreUintW i = 0u; i < HARENA_SPIKES; ++i)
    {
        ...
        if(m_afSpikeMax[i])
        {
            ...
            {
                // 
                     if(fCur < HARENA_SPIKE_LAUNCH)                 {const coreFloat fVal = MIN1((fCur)                                         * 5.0f); fHeight = LerpPara(-9.0f, -2.95f, -3.0f, fVal); vColor = LERPH3(vColor,            COLOR_SHIP_PURPLE, fVal);}
                else if(fCur < m_afSpikeMax[i] - HARENA_SPIKE_FADE) {const coreFloat fVal = MIN1((fCur - (HARENA_SPIKE_LAUNCH))                 * 5.0f); fHeight = LerpPara(-3.0f,  0.0f,  -0.5f, fVal); vColor = LERPH3(COLOR_SHIP_PURPLE, COLOR_SHIP_RED,    fVal);}
