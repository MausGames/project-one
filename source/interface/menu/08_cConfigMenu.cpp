...
void cConfigMenu::CheckValues()
{
    ...
    #define __CHECK_INPUT(n,a,c) {coreBool A = false; for(coreUintW i = 0u; i < (c); ++i) A = A || (a); bSave = bSave || A; m_aLabel[ENTRY_ ## n].SetColor3(A ? g_pMenu->GetHighlightColor() : COLOR_MENU_WHITE);}
    {
        ...
        __CHECK_VALUE(AUDIO_GLOBALVOLUME,    m_GlobalVolume          .GetCurValue()   != cConfigMenu::__VolumeToUint8(Core::Config->GetFloat(CORE_CONFIG_AUDIO_GLOBALVOLUME)))
        __CHECK_VALUE(AUDIO_MUSICVOLUME,     m_MusicVolume           .GetCurValue()   != cConfigMenu::__VolumeToUint8(Core::Config->GetFloat(CORE_CONFIG_AUDIO_MUSICVOLUME)))
        __CHECK_VALUE(AUDIO_EFFECTVOLUME,    m_EffectVolume          .GetCurValue()   != cConfigMenu::__VolumeToUint8(g_OldConfig.Audio.fEffectVolume))
        __CHECK_VALUE(AUDIO_AMBIENTVOLUME,   m_AmbientVolume         .GetCurValue()   != cConfigMenu::__VolumeToUint8(g_OldConfig.Audio.fAmbientVolume))
        __CHECK_VALUE(AUDIO_MENUVOLUME,      m_MenuVolume            .GetCurValue()   != cConfigMenu::__VolumeToUint8(g_OldConfig.Audio.fMenuVolume))
        __CHECK_VALUE(AUDIO_QUALITY,         m_AudioQuality          .GetCurValue()   != g_OldConfig.Audio.iQuality)
        __CHECK_VALUE(AUDIO_MODE,            m_AudioMode             .GetCurValue()   != Core::Config->GetInt(CORE_CONFIG_AUDIO_MODE))
        __CHECK_VALUE(AUDIO_3DSOUND,         m_3DSound               .GetCurValue()   != g_OldConfig.Audio.i3DSound)

        __CHECK_INPUT(INPUT_TYPE,            m_aInput[i].oType       .GetCurValue()   != g_OldConfig.Input.aiType       [i], MENU_CONFIG_INPUTS)
        __CHECK_INPUT(INPUT_RUMBLE,          m_aInput[i].oRumble     .GetCurValue()   != g_OldConfig.Input.aiRumble     [i], MENU_CONFIG_INPUTS)
        __CHECK_INPUT(INPUT_FIREMODE,        m_aInput[i].oFireMode   .GetCurValue()   != g_OldConfig.Input.aiFireMode   [i], MENU_CONFIG_INPUTS)
        __CHECK_INPUT(INPUT_CONTROLMODE,     m_aInput[i].oControlMode.GetCurValue()   != g_OldConfig.Input.aiControlMode[i], MENU_CONFIG_INPUTS)

        __CHECK_INPUT(INPUT_MOVEUP,          this->__RetrieveInputDirCurValue(i,  0u) != this->__RetrieveInputDirOldValue(i,  0u), INPUT_SETS)
        __CHECK_INPUT(INPUT_MOVELEFT,        this->__RetrieveInputDirCurValue(i,  1u) != this->__RetrieveInputDirOldValue(i,  1u), INPUT_SETS)
        __CHECK_INPUT(INPUT_MOVEDOWN,        this->__RetrieveInputDirCurValue(i,  2u) != this->__RetrieveInputDirOldValue(i,  2u), INPUT_SETS)
        __CHECK_INPUT(INPUT_MOVERIGHT,       this->__RetrieveInputDirCurValue(i,  3u) != this->__RetrieveInputDirOldValue(i,  3u), INPUT_SETS)

        __CHECK_INPUT(INPUT_ACTION1,         this->__RetrieveInputDirCurValue(i,  4u) != this->__RetrieveInputDirOldValue(i,  4u), INPUT_SETS)
        __CHECK_INPUT(INPUT_ACTION2,         this->__RetrieveInputDirCurValue(i,  5u) != this->__RetrieveInputDirOldValue(i,  5u), INPUT_SETS)
        __CHECK_INPUT(INPUT_ACTION3,         this->__RetrieveInputDirCurValue(i,  6u) != this->__RetrieveInputDirOldValue(i,  6u), INPUT_SETS)
        __CHECK_INPUT(INPUT_ACTION4,         this->__RetrieveInputDirCurValue(i,  7u) != this->__RetrieveInputDirOldValue(i,  7u), INPUT_SETS)
        __CHECK_INPUT(INPUT_ACTION5,         this->__RetrieveInputDirCurValue(i,  8u) != this->__RetrieveInputDirOldValue(i,  8u), INPUT_SETS)
        __CHECK_INPUT(INPUT_ACTION6,         this->__RetrieveInputDirCurValue(i,  9u) != this->__RetrieveInputDirOldValue(i,  9u), INPUT_SETS)
        __CHECK_INPUT(INPUT_ACTION7,         this->__RetrieveInputDirCurValue(i, 10u) != this->__RetrieveInputDirOldValue(i, 10u), INPUT_SETS)
        __CHECK_INPUT(INPUT_ACTION8,         this->__RetrieveInputDirCurValue(i, 11u) != this->__RetrieveInputDirOldValue(i, 11u), INPUT_SETS)
        ...
        __CHECK_VALUE(GAME_COMBATTEXT,       m_CombatText            .GetCurValue()   != g_OldConfig.Game.iCombatText)
        __CHECK_VALUE(GAME_BACKROTATION,     g_CurConfig.Game.iBackRotation           != g_OldConfig.Game.iBackRotation)
        __CHECK_VALUE(GAME_BACKSPEED,        g_CurConfig.Game.iBackSpeed              != g_OldConfig.Game.iBackSpeed)
        __CHECK_VALUE(GAME_UPDATEFREQ,       g_CurConfig.Game.iUpdateFreq             != g_OldConfig.Game.iUpdateFreq)
        __CHECK_VALUE(GAME_PUREMODE,         m_PureMode              .GetCurValue()   != g_OldConfig.Game.iPureMode)
        __CHECK_VALUE(GAME_LEADERBOARD,      m_Leaderboard           .GetCurValue()   != g_OldConfig.Game.iLeaderboard)
        __CHECK_VALUE(GAME_VERSION,          m_Version               .GetCurValue()   != g_OldConfig.Game.iVersion)

        if(!m_MirrorMode.GetOverride())
        {
            __CHECK_VALUE(GAME_MIRRORMODE, m_MirrorMode.GetCurValue() != g_OldConfig.Game.iMirrorMode)
        }
    }
    #undef __CHECK_VALUE
    #undef __CHECK_INPUT
