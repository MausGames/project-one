...
float GetSpec(const in vec3 v3ReflNormal, const in vec2 v2Offset)
{
    // lookup normal map
    vec3 v3TexNormal = coreTexture2D(0, v_av2TexCoord[0] + v2Offset).xyz;
    
    // calculate dot-3 bump factor
    vec3 v3BumpNormal = v3TexNormal * 2.0 - 1.0;
...
void FragmentMain()
{
    ...
    // lookup normal map and depth map
    ...
    vec3  v3BumpNormal   = v3TexNormal * 2.0 - 1.0;
          v3BumpNormal   = normalize(vec3(v3BumpNormal.xy, v3BumpNormal.z * 0.4));
    ...
    // 
    const float A = 0.001;
    float v1ReflFactor = (GetSpec(v3ReflNormal, vec2(0.0, 0.0))          +
                          GetSpec(v3ReflNormal, vec2(  A, 0.0))          + 
                          GetSpec(v3ReflNormal, vec2( -A, 0.0))          + 
                          GetSpec(v3ReflNormal, vec2(0.0,   A))          +
                          GetSpec(v3ReflNormal, vec2(0.0,  -A))          +
                          GetSpec(v3ReflNormal, vec2(  A,   A) * 0.7071) + 
                          GetSpec(v3ReflNormal, vec2( -A,   A) * 0.7071) + 
                          GetSpec(v3ReflNormal, vec2( -A,  -A) * 0.7071) +
                          GetSpec(v3ReflNormal, vec2(  A,  -A) * 0.7071)) * 0.25;
