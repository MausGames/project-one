...
void cNevoMission::__SetupOwn()
{
            ...
            if(g_pGame->IsTask())
            {
                ...
                if(bIsFree)
                {
                    ...
                    if(pEnemy->ReachedDeath() && (!iTileDone || !iTileWait))
                    {
                        ...
                        {
                            g_pGame->GetCombatText()->DrawProgress(iFreeCount, ARRAY_SIZE(aiFree), pEnemy->GetPosition());
                            g_pSpecialEffects->PlaySound(pEnemy->GetPosition(), 1.0f, SPECIAL_SOUND_PROGRESS(iFreeCount, ARRAY_SIZE(aiFree)), SOUND_ITEM_02);
                        }
                    }
                }
            }
    ...
    STAGE_MAIN({TAKE_ALWAYS, 2u})
    {
        ...
        STAGE_GET_START(14u + NEVO_BOMBS * 3u + GAME_PLAYERS)
            ...
            STAGE_GET_FLOAT_ARRAY(afStill, GAME_PLAYERS)
        STAGE_GET_END
        ...
        else
        {
            ...
            for(coreUintW i = 0u; i < NEVO_BOMBS; ++i)
            {
                ...
                if(this->GetBombGone(i))
                {
                    ...
                    g_pSpecialEffects->RumblePlayer(NULL, (coreMath::PopCount(m_iBombGone) > 3u) ? SPECIAL_RUMBLE_BIG : SPECIAL_RUMBLE_SMALL, 250u);
                }
            }
            ...
            {
                if(STAGE_TICK_FREE2(0.25f, -0.5f))   // 2
                {
                ...
                for(coreUintW i = 0u; i < NEVO_CHIPS; ++i)
                {
                    ...
                    STAGE_FOREACH_PLAYER(pPlayer, j)
                    {
                        ...
                        if(vDiff.LengthSq() < POW2(5.0f))
                        {
                            ...
                            if(++iChipCollect >= NEVO_CHIPS)
                            {
                                STAGE_BADGE(0u, BADGE_EASY, pChip->GetPosition())
                            }
                            else
                            {
                                g_pGame->GetCombatText()->DrawProgress(iChipCollect, NEVO_CHIPS, pChip->GetPosition());
                                g_pSpecialEffects->PlaySound(pChip->GetPosition(), 1.0f, SPECIAL_SOUND_PROGRESS(iChipCollect, NEVO_CHIPS), SOUND_ITEM_01);
                            }
                        }
                    });
                }
                ...
                if(iCurEnemy == i)
                {
                    if(pEnemy->ReachedDeath())
                    {
                        ...
                        if(g_pGame->IsTask())
                        {
                            ...
                            {
                                if(++iBackstabCount >= 20u)
                                {
                                    STAGE_BADGE(1u, BADGE_NORMAL, pPlayer->GetPosition())
                                }
                                else
                                {
                                    g_pGame->GetCombatText()->DrawCountdown(iBackstabCount, 20u, pPlayer->GetPosition());
                                    g_pSpecialEffects->PlaySound(pPlayer->GetPosition(), 1.0f, SPECIAL_SOUND_PROGRESS(iBackstabCount, 20u), SOUND_ITEM_02);
                                }
                            }
                        }
                    }
                    ...
                    fRota = (((1.0f - pEnemy->GetCurHealthPct()) + I_TO_F(i)) * RCP(I_TO_F(pSquad1->GetNumEnemies()))) * (2.0f*PI);
                }
                else
                {
                    pEnemy->SetPosition(coreVector3(HIDDEN_POS, 0.0f));
    ...
    STAGE_MAIN({TAKE_ALWAYS, 3u})
    {
        ...
        {
            return g_pGame->IsTask() && std::memchr(aiFake, iIndex, ARRAY_SIZE(aiFake));
        };
        ...
        if(STAGE_CLEARED)
        {
            ...
            {
                if(iTurnCount <= 20u + 1u) STAGE_BADGE(3u, BADGE_ACHIEVEMENT, g_pGame->FindPlayerDual(0u)->GetPosition())
            }
        }
        ...
        if(g_pGame->IsTask() && (m_iStageSub >= 10u))
        {
            if(fLockTime >= 0.0f)
            {
                constexpr coreFloat fLimit = 2.0f;
                ...
                {
                    fLockTime = -1.0f;
                    STAGE_BADGE(0u, BADGE_EASY, coreVector3(0.0f,0.0f,0.0f))
                }
                else if(fLockTime >= fLimit)
                {
                    fLockTime = -1.0f;
                }
                else
        ...
        STAGE_COLL_ENEMY_BULLET(pEnemy, pBullet, vIntersection, bFirstHit, COLL_THIS, COLL_VAL(pSquad1), COLL_VAL(aiArrowMap), COLL_REF(iFakeCount), COLL_VAL(nEntryFunc), COLL_VAL(pHelper))
        {
            ...
            if(iEntry)
            {
                ...
                {
                    if(++iFakeCount >= ARRAY_SIZE(aiFake))
                    {
                        STAGE_BADGE(1u, BADGE_NORMAL, pEnemy->GetPosition())
                    }
                    else
                    {
                        g_pGame->GetCombatText()->DrawProgress(iFakeCount, ARRAY_SIZE(aiFake), pEnemy->GetPosition());
                        g_pSpecialEffects->PlaySound(pEnemy->GetPosition(), 1.0f, SPECIAL_SOUND_PROGRESS(iFakeCount, ARRAY_SIZE(aiFake)), SOUND_ITEM_02);
                    }
                }
        ...
        if(STAGE_BEGINNING)
        {
            pBackground->GetOutdoor()->LerpHeight(0.2f, -17.0f);
        }
    ...
    STAGE_MAIN({TAKE_ALWAYS, 4u})
    {
        ...
        if(g_pGame->IsTaskExtra() && (m_iStageSub == 5u))
        {
            ...
            for(coreUintW i = 0u; i < ARRAY_SIZE(avMarker); ++i)
            {
                ...
                {
                    STAGE_BADGE(2u, BADGE_HARD, coreVector3(vMarkerPos, 0.0f))
                }
            }
        }
        ...
        if(m_iStageSub == 5u)
        {
            ...
            {
                const coreVector2 vPos = coreVector2( 1.2f,0.3f) * FOREGROUND_AREA;
        ...
        STAGE_FOREACH_ENEMY(pSquad1, pEnemy, i)
        {
            ...
            else if(i < 12u)
            {
                ...
                const coreVector2 vFactor = coreVector2(1.0f,1.0f);
                const coreVector2 vOffset = coreVector2((i < 8u) ? 0.7f : (I_TO_F(i - 8u) * 0.3f - 0.9f), 0.0f);
        ...
        g_pGame->GetBulletManagerEnemy()->ForEachBulletTyped<cGrowBullet>([&](cGrowBullet* OUTPUT pBullet)
        {
            ...
            if(iDamage > 5)
            {
                if(iDamage > (10 + 20 * g_pGame->GetNumPlayers()))
                {
                    ...
                    {
                        g_pGame->GetCombatText()->DrawProgress(iBubbleCount, 5u, pBullet->GetPosition());
                        g_pSpecialEffects->PlaySound(pBullet->GetPosition(), 1.0f, SPECIAL_SOUND_PROGRESS(iBubbleCount, 5u), SOUND_ITEM_01);
                    }
        ...
        if(g_pGame->IsTask() && apBig[0] && apBig[1])
        {
            ...
            STAGE_FOREACH_PLAYER(pPlayer, i)
            {
                ...
                {
                    STAGE_BADGE(1u, BADGE_NORMAL, coreVector3(pPlayer->GetPosition().xy(), 0.0f))
                }
            });
        }
    ...
    STAGE_PRE()
    {
        ...
        g_pGame->GetBulletManagerEnemy()->PrefetchBullet<cGrowBullet>();
    });
    ...
    // ################################################################
    // story
    if(m_bStory && !g_bDemoVersion) STAGE_MAIN({TAKE_ALWAYS, 5u})
    {
        ...
        if(STAGE_TIME_POINT(2.0f))
        {
            ...
            m_pNightmareSound->PlayRelative(this, 0.3f, 1.0f, false, SOUND_EFFECT, CORE_AUDIO_EFFECT_NONE);
        }
